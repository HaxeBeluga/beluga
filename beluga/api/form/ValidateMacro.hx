...
import beluga.macro_tool.ExprUtils;
import beluga.macro_tool.TypeUtils;
...
class ValidateMacro {
    private static function generateCondition(field_name : String, rule : MetadataEntry) : Expr {
        ...
        if (Reflect.isFunction(Reflect.field(beluga.api.form.DataChecker, condition_name)) == false) {
        ...
        {
            if (beluga.api.form.DataChecker.$condition_name($a{condition_arg}) == false) {
